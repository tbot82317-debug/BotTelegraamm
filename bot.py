...
async def recon(url):
    """HTML صفحه رو چاپ می‌کنه، سعی می‌کنه بازی رو استارت کنه، و از canvas عکس می‌گیره
    (به‌صورت base64 توی لاگ) تا رنگ/موقعیت شاخه‌ها رو دقیق کالیبره کنیم."""
    async with async_playwright() as p:
        ...
        if box:
            # کلیک روی دکمه پلی (که خودش داخل canvas رسم شده، حدود ۸۵٪ ارتفاع)
        ...
        print("===== BODY CLASS AFTER CLICK =====")
        body_class = await page.eval_on_selector("body", "el => document.getElementById('page_wrap').className")
        print("page_wrap class:", body_class)

        # اسکرین‌شات از canvas بازی
        png_bytes = await canvas.screenshot()
        b64 = __import__("base64").b64encode(png_bytes).decode()
        print(f"===== CANVAS SCREENSHOT BASE64 START (len={len(b64)}) =====")
        for i in range(0, len(b64), 2000):
            print(f"B64|{i}|{b64[i:i+2000]}")
        print("===== CANVAS SCREENSHOT BASE64 END =====")
