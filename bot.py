...
async def recon(url):
    ...
    async with async_playwright() as p:
        ...
        # نقاط نمونه‌برداری: چند ستون (نسبت به مرکز ۲۰۶) در چند ردیف مختلف ارتفاع
        xs = [70, 120, 160, 206, 250, 290, 340]
        ys = [380, 430, 480, 530, 580, 620]
        ...
        for tick in range(30):
            ...
            try:
                samples = await page.evaluate(sample_js, coords)
            except Exception as e:
                samples = None
                print(f"t={tick} خطا در خوندن پیکسل‌ها: {e}")

            if samples:
                compact = ";".join(f"{x},{y}:{r},{g},{b}" for x, y, r, g, b in samples)
                print(f"t={tick} class={pw_class} px={compact}")
